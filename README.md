...
- **Share image**: `app/opengraph-image.tsx` generates a branded 1200x630 PNG at build time (Playfair fetched from Google Fonts, falling back to a system serif if the fetch fails). Product pages override it with the real product photo, falling back to the site image when a product has no photos yet.
