...
> **Jest + React Testing Library** are installed (`jest.config.ts`, `jest.setup.ts`). Specs today cover the Excel import (`components/admin/import/__tests__/`) — both its pure modules and every component of that screen; see that folder's `README.md` for the layout — the full checkout flow (`components/checkout/__tests__/`: `CheckoutContext`, `usePlaceOrder`, `CouponField`, `OrderTotals`, `ShippingOptions`, `UserDetails`, `Stepper`, `checkoutErrors`), plus a dozen pure modules: `lib/domain/__tests__/idempotency.test.ts`, `publicOrderToken.test.ts`, `catalogFilters.test.ts`, `cart.test.ts`, `categories.test.ts`, `brand.test.ts`, `shipmentStatus.test.ts`, `components/admin/orders/__tests__/shipmentLabel.test.ts`, `components/admin/coupons/__tests__/couponStatus.test.ts`, `components/admin/expenses/__tests__/expenseStatus.test.ts` and `components/order/__tests__/orderTimeline.test.ts`. See `ROADMAP-TEST-COVERAGE.md` for what's still missing (order-tracking, outlet, auth, admin components).
