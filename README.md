...
- **@stripe/stripe-js** + **@stripe/react-stripe-js** — checkout payment gateway (Payment Element, card only)
- **framer-motion** — animations (drawers, transitions, carousels)
- **recharts** — admin dashboard charts
- **sileo** — toast notifications (admin panel only)
- **pnpm** as package manager

> **Jest + React Testing Library** (`jest.config.ts`, `jest.setup.ts`) cover the full checkout flow, the public order-tracking flow, the public outlet/catalog listing, the Excel import screen, auth, every admin section (órdenes, productos, cupones, gastos, dashboard/reportes/config and their container components), shared UI + home, the whole `lib/api/` layer (one spec per module, near-total coverage), and about a dozen pure `lib/domain/` modules. See `CLAUDE.md`'s "Testing" section for the file-by-file breakdown (and a couple of real bugs the specs caught) and each folder's own `README.md` for area-specific notes; `ROADMAP-TEST-COVERAGE.md` tracks what's still missing (`AuthShell`, `BrandSection`, the `ProductSection`/`ImportSection` containers).
...
components/
  ...
  checkout/       # 4-step checkout wizard, incl. ShippingOptions (live Skydropx rate quoting),
                  #   CouponField (discount code, validated server-side), checkoutErrors.ts
                  #   (pure order/payment error mapping, with specs) and PaymentSection
                  #   (the Payment Element, bare — see CLAUDE.md "Pagos (Stripe)")
  ...
  admin/          # Sidebar, types.ts + sections/ (Brand, Product, Import, Coupons, Orders,
                  ...
                  #   dependencies, labels) + components; specs live in import/__tests__/ (see its
                  #   README.md)
lib/
  ...
  stripe/         # getStripe() singleton (Stripe.js) + isStripeTestMode() + appearance.ts
                  #   (el Design System para el iframe del Payment Element)
...
Env: set `NEXT_PUBLIC_API_URL` to the backend URL (defaults to `/api`), `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY` to Stripe's **publishable** key (`pk_test_…` in sandbox, `pk_live_…` in production, same account as the backend's `STRIPE_SECRET_KEY` — its prefix is also what drives the "Modo de prueba" badge in the checkout), and `NEXT_PUBLIC_SITE_URL` to the site's public origin (see SEO below). `NEXT_PUBLIC_*` vars are inlined at build time — restart the dev server after changing them. See `BACKEND.md` for the full API contract.
...
Shipping is quoted **live** against Skydropx from checkout step 3 (see above) — the backend is the authority and re-queries the chosen quote when creating the order, so the amount shown always matches the amount charged. `lib/domain/cart.ts` deliberately does **not** compute shipping itself (`CartTotals.shipping` stays `number | null` until a real quote exists) — a local flat-rate copy was removed once it started undercharging vs. the backend's per-box pricing, and must not be reintroduced. See `CLAUDE.md`'s "Shipping" section for the full contract.
...
- **Reportes** — monthly sales history feeding an auto-scaling replenishment forecast (simple average → weighted+trend → Holt exponential smoothing, depending on history depth). Reposición exports to CSV (a shopping list meant to be filtered/priced in a spreadsheet); Ventas exports to a printable sheet instead, with a KPI summary and category breakdown the CSV never had.
