...
// `POST /api/orders` devuelve TRES 409 distintos y cada uno pide una
// recuperación diferente: sin stock (el usuario corrige el carrito), cotización
// expirada (hay que re-cotizar) y clave de idempotencia reusada (hay que rotar
// la clave). El status no los distingue — solo el `message` — así que confundir
// dos de ellos deja al comprador reintentando en bucle contra el mismo error.
...
describe("los tres 409 no se confunden entre sí", () => {
  ...
  it("no reacciona a un 409 sin cuerpo ni a un error que no es de axios", () => {
    for (const detector of [isRateExpiredError, isIdempotencyKeyConflict]) {
