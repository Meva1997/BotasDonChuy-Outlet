        ...
        >
          {NAV_LINKS.map(({ href, label }) => (
            <Link
              key={href}
              href={href}
              className="relative text-xs tracking-[0.2em] uppercase text-amber-100/60 hover:text-amber-100 transition-colors duration-200 group"
            >
              {label}
              <span className="absolute left-0 -bottom-1.5 w-0 h-px bg-amber-400/70 group-hover:w-full transition-all duration-300" />
            </Link>
          ))}
        </nav>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="md:hidden absolute top-full left-0 right-0 bg-tobacco-950 border-b border-amber-900/40 overflow-hidden"
          >
