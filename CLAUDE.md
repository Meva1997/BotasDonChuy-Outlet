...
**`SalesReport`'s net profit is not the dashboard's GANANCIA OPERATIVA, and can't be.** `MonthlyReport.byProduct[].revenue` is `unitsSold × current salePrice` (backend `reports.service.ts`) — merchandise at list price, so it reflects neither coupon discounts nor later price edits, and it does **not** include the shipping charged to the customer. That's exactly why `ExpenseMonth.shippingCost` is **deliberately not subtracted here**: the dashboard subtracts the guía because its `ingresos` is `order.total`, which *does* carry the shipping in; netting it against a base that never included it would charge the same peso twice. Both facts are stated on screen and on the printed sheet — don't "fix" the discrepancy by subtracting shipping (a spec guards it).
