...
- **Testing** — Jest + React Testing Library + user-event (`next/jest`; `jest.setup.ts` stubs `window.matchMedia`, needed by framer-motion's `useReducedMotion()`, and `window.IntersectionObserver`, needed by framer-motion's `whileInView` — e.g. `Hero.tsx`/`Footer.tsx`). `testMatch` is scoped to `**/*.test.{ts,tsx}` (not "anything under `__tests__/`") so fixtures/helpers can live alongside specs. **`jest.mock("@/...")` does not resolve** — Next's alias transform only rewrites real `import`/`require` specifiers, not the string literal passed to `jest.mock()`, which goes through Jest's own resolver instead (no `@/*` mapper configured). Use a relative path in `jest.mock()` calls (e.g. `jest.mock("../../../lib/api/orders", ...)`) even though the rest of the file imports the same module with `@/`. **TanStack Query notifies subscribers on a macrotask**, so any assertion of *absence* right after a query state change (e.g. "a failed refetch must NOT show the error block") needs a `setTimeout(…, 0)` flush inside the same `act()` — without it the assertion runs against the pre-change DOM and passes either way (see `OutletView.test.tsx`'s refetch case). Coverage today: **the full checkout flow** (`components/checkout/__tests__/` — `CheckoutContext`, `usePlaceOrder`, `CouponField`, `OrderTotals`, `ShippingOptions`, `UserDetails`, `Stepper`, `checkoutErrors`, own `helpers/factories.ts` + `helpers/apiError.ts` + `helpers/render.tsx` for the `QueryClientProvider`+`CheckoutProvider` wrapper), **the full public order-tracking flow** (`components/order/__tests__/` — `OrderLookupForm`, `OrderStatusTimeline`, `TrackedOrderItems`, `OrderTracking`, `orderTimeline`, own `helpers/factories.ts` for `PublicOrder`/`PublicOrderItem` fixtures), **the full public outlet/catalog listing** (`components/outlet/__tests__/` — `OutletFilters`, `OutletView`, `EmptyState`, `OutletCard`, `OutletPagination`, own `helpers/factories.ts` for `Product`/`ProductsResult` fixtures; `next/navigation`'s `useRouter`/`useSearchParams` mocked directly per-file, same pattern as `OrderLookupForm.test.tsx` — the `<Suspense fallback={<OutletSkeleton />}>` boundary itself lives in `app/(public)/outlet/page.tsx` and its category siblings, outside what a component test can exercise), **the full Excel-import screen** (`components/admin/import/__tests__/` — the other place with component specs), **auth** (`components/auth/__tests__/` — `LoginForm`, `ForgotPasswordForm` tested as one integrated flow with the `ResetCodeForm`/`NewPasswordForm` steps it renders (one exception: `NewPasswordForm` is mounted standalone for the `onExpired`-absent branch, unreachable through the wizard, which always passes it), `CodeInput`, `AdminGuard`; own `helpers/render.tsx` for a bare `QueryClientProvider` wrapper, `helpers/apiError.ts` duplicated from checkout's — a `__tests__/` doesn't import across sibling folders. `CodeInput`'s auto-advance had a real bug caught while writing these specs: `handleFocus` read `value` from a stale render closure inside the synchronous `focus` event it triggers itself, so it bounced back to the same box instead of the next one — typing a code digit-by-digit silently corrupted it. Fixed with a ref updated synchronously by `setValue()` at commit time — see `CodeInput.tsx`), and **Admin: Órdenes** (`components/admin/orders/__tests__/` — `StatusBadges` (the disjoint-color invariant between `STATUS_META`/`PAYMENT_META`, verified by breaking the source and confirming the test fails), `OrdersPagination`, `OrdersTable` (row selection, `labelNote`'s four branches; the `xl:hidden` cards and `hidden xl:block` table both render in jsdom at once, so unique-text assertions go through `within(screen.getByRole("table"))` — and the cards' handlers are their own, so they need their own clicks despite rendering in every test), `OrderDetailModal` (coupon row ordering, `size === 0` dash, cancel/refund visibility, the emitted-refund block, the dropoff notice, forward-only status advance, all 5 `shipmentLabel.ts` states including the `force` confirmation, and the tracking-code rotation — asserted as visible in **all five** order statuses, since it's the one action with no 409, and asserted to invalidate nothing and call no `onOrderUpdated`, which is what keeps it from swallowing the next polling toast), `PrintPendingOrders` (Fase 26 — the unpaginated `getAdminOrders(1, PRINT_PER_PAGE, undefined, "pendientes_envio")` call, the printable block's client/address/item fields incl. shipping cost and total, the product code resolved by cross-referencing `getAdminProducts()` and omitted when a product has none, loading/error states, `window.print()` mocked since jsdom doesn't implement it), own `helpers/factories.ts`/`apiError.ts`/`render.tsx` — see that folder's `README.md`), and **Admin: Productos** (`components/admin/products/__tests__/` — `ProductForm` (the "Maneja tallas" toggle never sends `sizes` and `stockQuantity` together, the up-to-3-image gallery incl. type/size/cupo rejection and precedence, a mid-flight image-upload failure retries with `updateProduct` instead of recreating and never re-deletes an image already gone, images deleted before uploaded (frees quota within the 3-image cap), per-category dimension reset only on untouched fields and never while editing, 400/404/502 error mapping), `ProductCategoryView` ("Cargar más" pagination, view/edit navigation, inline delete confirm/cancel/pending/error), `ProductDetailModal` (grouped sizes, the "Tallas" block fully absent — not empty — when `hasSizes: false`), `notices` (the shared `deleteNotice`/`saveNotice` copy), own `helpers/factories.ts`/`apiError.ts`/`render.tsx` — see that folder's `README.md`. A real bug was caught here too: `ProductForm`'s unmount cleanup for `blob:` image previews had `useEffect(..., [])` closing over `newImages` from the mount render (always empty), so it never actually revoked anything added during the session — same stale-closure shape as the `CodeInput` bug above. Fixed with a ref kept in sync by a second effect), and **Admin: Cupones y Gastos** (`components/admin/coupons/__tests__/` — `CouponForm` (every cross-field rule mirroring the backend's `couponFormSchema.superRefine`, including the max-discount field that stays *disabled but not cleared* when switching to a fixed-amount coupon so its stale value still blocks submit; the "vaciar un campo no lo borra" notice in singular/plural), `CouponsTable` ("Cancelar" calls `onToggleActive` — a `PUT { active:false }`, never a delete — and "Reactivar" takes its place once cancelled; the redemption-divergence notice in singular/plural), own `helpers/factories.ts`/`render.tsx` — see that folder's `README.md`; and `components/admin/expenses/__tests__/` — `ExpenseForm` (the amount fields exist only on create, never on edit — that's `ExpenseAmountForm`'s job), `ExpenseAmountForm` (every submit appends a dated version, verified across an increase, a decrease, and the oldest entry with no prior version to diff against), `ExpenseHistory` (its own lazy query's loading/error/empty states, the month selector, the derived shipping line), `ExpensesTable` (the twin of `CouponsTable`: "Dar de baja" is `PUT { active:false }`, never a delete; "Cambiar precio" is its own action, separate from edit, because the amount is a dated version and not a column; `RunRateCell` renders "—" and never "$0.00", or an $8,000 one-off reads as free), `ExpenseSummaryCard` (`monthlyRunRate` and `upcomingTotal` as independent figures — adding them would double-count; the four-category cap; the date-grouped timeline and its "ver más" trim), `ShippingCostNote` (no controls, shown but never summed into totals — tested from **both** its consumers, the history month detail and the summary card, since being shared is the whole point), `ExpenseStateBadge` (five states, disjoint tones), own `helpers/factories.ts`/`render.tsx` — see that folder's `README.md`. A real bug was caught here too: `ExpenseForm`'s "Fecha de término" field — and its `FieldError` — only exist in the DOM while `frequency !== "once"`, but react-hook-form doesn't unregister an unmounted field by default, so a date typed before switching to "once" silently survived and kept failing `superRefine` on submit with zero visible feedback. Fixed by clearing `endsAt` in a `useEffect` the moment `frequency` becomes `"once"`, instead of just hiding the input. The two `*Section.tsx` containers are deliberately out of scope here, same as `OrdersSection` — they own the write calls the subcomponents only receive as props; covered in Fase 9), and **Admin: Dashboard, Reportes, Config y los contenedores** (Fase 9 — `components/admin/data/__tests__/` (`KpiGrid`, whose trend color must follow `trend.positive` and never the number, since `COSTO DE ENVÍO` arrives with that flag deliberately inverted; `SalesTable`, the only client-paginated/filtered table in the panel, whose row profit subtracts shipping; `InventoryTable`'s three stock tiers and card-only pagination; `RevenueChart`), `components/admin/reports/__tests__/` (Reposición's CSV asserted on the real Blob — filename, BOM, RFC 4180 escaping, full rows — and Ventas' printable sheet, which replaced its CSV, asserted on the mounted `#print-reporte-ventas` block; plus the two ways `trendVsPrev` can legitimately return null, the four "—" that keep a zero or the 999 sentinel from reading as data, and the cross-check against `getExpenseHistory` — including the partial-month label and the deliberate non-subtraction of `shippingCost`), `components/admin/config/__tests__/` (`currentPassword` required for any change and the new-password keys only travelling when typed; this route's 401 means "wrong password", not "session expired"; `AdminsCard`'s string-vs-number id comparison, which is what stops the owner being offered a delete button on their own account), and `components/admin/sections/__tests__/` (the containers that decide what gets sent: `OrdersSection`'s 20/5 `perPage` via `matchMedia`, the four suppressions of the Sileo toast, and (Fase 25) the 4 status tabs — default tab on mount, `estado` per tab incl. the "Enviados" tab requesting `estado=enviados` and its empty-message "No hay pedidos enviados sin entregar", "Todos" omitting the param, tab+date combining in one query, and the tab resetting `page` to 1 and starting a fresh polling-toast snapshot, plus (Fase 26) the "Imprimir pendientes" button only rendering on the "Pendientes de enviar" tab, `CouponsSection` vs. `ExpensesSection` — near-twins whose real difference is that a coupon invalidates only its own cache while an expense also invalidates `dashboardKeys`, and both report what the backend *did* (`deactivated: true`) rather than what the UI asked for, `ReportsSection`'s default month (the last **complete** one) and its reconciliation, `DataSection`, `ConfigSection`). Each folder has its own `README.md`. **`RevenueChart.test.tsx` mocks `recharts.ResponsiveContainer`** to clone its child with fixed dimensions — recharts measures 0×0 in jsdom and renders no axes at all, which is why `ExpenseHistory` had written `formatY` off as uncoverable; with the mock the Y-axis ticks render and both branches are read from the DOM (the same trick would close that older gap). Two real bugs were caught here: `OrdersSection`'s polling toast switched itself off — `isManualRefreshRef` is cleared inside the signature-comparing effect, which depended on `[data, …]`, but TanStack Query's structural sharing keeps the same `data` reference when a refetch returns identical content, so a manual refresh that found nothing new (the common case) left the flag armed forever and the *next* automatic refetch swallowed its toast; fixed by adding `dataUpdatedAt` to the deps. And both config cards had eight `<label>`s that were mere siblings of their inputs (no `htmlFor`, no nesting), so screen readers had no name for any field and clicking a label didn't focus it — surfaced by `getByLabelText` failing, fixed with `htmlFor`/`id`, the pattern every other admin form already used), and **shared UI + home** (Fase 10 —
`components/ui/__tests__/` (`Cart` — the drawer's per-size stock cap on the `+` button, `size === 0`
hiding the talla row, totals, and the render-time state adjustment that resets `navigating` when
the cart reopens; `ImageCarousel` — the 0/1/2+ image branches, wrap-around in both directions,
keyboard nav only with 2+ images; `FormControls` — `TextField`/`SelectField`'s error/placeholder
branches and ref forwarding) and `components/home/__tests__/` (`Hero` — the three per-category
`getProducts({ categoria, perPage: 1 })` queries, reading only `total`; `NavHeader` — mobile menu
open/close incl. click-outside; `Footer`, `CategoryCard`), own `helpers/factories.ts` per folder
(`home/`'s is a deliberate copy of `ui/`'s — a `__tests__/` never imports from a sibling folder,
same rule as the `apiError.ts` duplicated between `checkout/` and `auth/`) — see each folder's
`README.md`. None of these mount `<BrandProvider>`: `BrandContext`'s own default
value already equals the resolved fallback (`resolveBrand(null)`), so `useBrand()` returns real
copy without a query. A real bug was found but deliberately **not** fixed here: `Cart.tsx`'s
"Cargando..." checkout-loading state can never actually render — `setNavigating(true)` and
`closeCart()` fire in the same synchronous handler, so `AnimatePresence` animates the panel's exit
using the last tree rendered *before* the click; fixing it is a UX call (should the cart stay open
during navigation?) out of scope for a test pass — see `components/ui/__tests__/README.md`), plus a
handful of pure modules elsewhere: `lib/domain/__tests__/idempotency.test.ts`, `orders/__tests__/shipmentLabel.test.ts`, `publicOrderToken.test.ts`, `catalogFilters.test.ts`, `couponStatus.test.ts`, `expenseStatus.test.ts`, `cart.test.ts`, `categories.test.ts`, `brand.test.ts`, `shipmentStatus.test.ts` (see `ROADMAP-TEST-COVERAGE.md` for the phased plan covering the rest). No specs yet for `AuthShell`, `BrandSection` or `ProductSection`/`ImportSection`'s containers (their subcomponents are covered). Import tests are grouped by pipeline phase (`pure/`, `intake/`, `review/`, `editing/`, `presentation/`, `commit/` + `helpers/`) — see `components/admin/import/__tests__/README.md` before adding more. No snapshots (would break on every Tailwind tweak). No Playwright/e2e — don't reintroduce without being asked.
...
components/
  ...
  legal/          # TermsConditions, PrivacyPolicy, ShippingInfo — static, sobre LegalLayout.
                  ...
                  #   **Invariante: el texto legal describe lo que el sistema HACE, no lo que
                  #   quisiéramos que hiciera.** La conservación de datos dice 5 años fiscales porque
                  #   no existe rutina de borrado de `Order` (antes prometía 30 días — era falso).
  ...
  admin/          # Sidebar + types.ts (AdminSection) + sections/ (8 tabs) + per-tab subcomponent
                  ...
                  #     Fase 25: 4 status tabs (Pendientes de enviar · Enviados · Entregados · Todos,
                  #     default "Pendientes de enviar") map to `estado` (`AdminOrderEstadoFilter`, in
                  #     lib/api/adminOrders.ts) — `"pendientes_envio"|"enviados"|"entregados"`, "Enviados"
                  #     maps to `Order.status === "shipped"`, filtered 100% in
                  #     SQL by the backend; "Todos" OMITS the param rather than sending `"todos"`.
                  #     Tab lives in `adminOrderKeys.list`'s queryKey (alongside page/perPage/date) so
                  #     switching tabs is a real refetch, never a client-side filter, and resets `page`
                  #     to 1 like the date filter already does. Combines freely with `date` — both are
                  #     independent backend params.
                  ...
                  #     (not manual, not first load) finds something changed. The snapshot used for that
                  #     diff is keyed by page+perPage+date+tab, so switching tabs (or page/date) always
                  #     starts a fresh photo instead of comparing across unrelated filtered views.
                  ...
                  #     PrintPendingOrders (Fase 26): button visible only on the "Pendientes de enviar"
                  #     tab, and only when there's actually something to print — `OrdersSection` runs its
                  #     own cheap existence check (`getAdminOrders(1, 1, undefined, "pendientes_envio")`,
                  #     same perPage:1-just-read-`total` trick as `Hero.tsx`) and swaps the button for a
                  #     "Nada pendiente por imprimir" notice when that total is 0. That check can't reuse
                  #     the tab's own `data.total` because the print button ignores `selectedDay` while
                  #     the table query doesn't — a day with zero pending orders would otherwise hide the
                  #     button even with pendings sitting on other days. Once shown, the button always
                  #     prints ALL pending-to-ship orders in one shot, ignoring the `date` filter and
                  #     pagination on screen. Fetches via a direct `getAdminOrders(1,
                  #     PRINT_PER_PAGE, undefined, "pendientes_envio")` call (bypasses `adminOrderKeys.list`
                  #     on purpose, to not mix this unpaginated snapshot into the table/polling cache) —
                  #     safe because the backend only clamps `page`, never `perPage`. Product `code` isn't
                  #     on `AdminOrderItem` (order items only snapshot name/size/price/cost), so it's
                  #     resolved by cross-referencing `item.productId` against a full `getAdminProducts()`
                  #     fetch (via `queryClient.ensureQueryData`, reusing the cache if "Productos" was
                  #     already visited). Renders a light (black-on-white) `#print-pedidos-pendientes`
                  #     block — the app's dark theme would waste ink/be illegible on paper — isolated for
                  #     printing by the `@media print` rule in `app/globals.css` (hides everything else via
                  ...
                  #     calls POST /admin/orders/:id/rotate-token. It exists because the Aviso de
                  #     Privacidad promises the buyer an exposed tracking code can be invalidated —
                  #     before this there was no way to honour that outside the database. **It is the
                  #     only action in this modal with NO 409**, so unlike cancel/label/status it
                  #     renders in EVERY order status: a leaked link has to be killable even on a
                  #     delivered or cancelled order. Two-step confirm (same shape as
                  #     confirmingCancel/confirmingRetry) because it locks the legitimate buyer out
                  #     until the backend's automatic email reaches them. Deliberately does NOT
                  #     invalidate `adminOrderKeys` NOR call `onOrderUpdated`: no admin view paints
                  #     `publicToken` (nothing goes stale on screen), and `onOrderUpdated` also arms
                  #     OrdersSection's `isManualRefreshRef` — the polling-toast suppressor — even
                  #     though rotation touches no `orderSignature` field, which would swallow the
                  #     toast for the NEXT real webhook change. Success is confirmed inline, not with
                  #     a Sileo toast: the Toaster lives in app/admin/layout.tsx, ties on z-index with
...
**Two different exports, on purpose:**

- **Reposición → CSV.** `csvField()` (RFC 4180 escaping) + BOM for Excel accents, `reposicion-<YYYY-MM>.csv`. It's a shopping list that gets filtered, sorted and priced in a spreadsheet — it has to stay machine-readable.
- **Ventas → hoja impresa.** The CSV export (and `SalesReport`'s own copy of `csvField`) was **replaced** by a printable document: a light-themed `#print-reporte-ventas` block isolated by the `@media print` rule in `app/globals.css`, same pattern as `PrintPendingOrders.tsx`. It gathers what the CSV left out — KPI summary, per-product average unit price (`revenue/unitsSold`, which doesn't exist on screen) and the category breakdown. Print is triggered by a **counter**, not a boolean: a second click on an already-mounted block would be a no-op `setState`, and a browser that never dispatches `afterprint` would leave the button dead.

**`SalesReport`'s net profit is not the dashboard's GANANCIA OPERATIVA, and can't be — even though both now subtract shipping.** `MonthlyReport.byProduct[].revenue` is `unitsSold × current salePrice` (backend `reports.service.ts`) — merchandise at list price, so it reflects neither coupon discounts nor later price edits. **Utilidad bruta** stays shipping-free on purpose (`totalRevenue` never included the guía cobrada, so subtracting it there would double-charge the same peso). **Utilidad neta**, though, DOES subtract `ExpenseMonth.shippingCost.amount` on top of `gastosOperativos` (own KPI tile "Gasto en paquetería", shown separately and fed into the neta calc) — a deliberate reversal of the original Fase 22 design, requested once the owner needed the guía to actually hit the bottom line here too, not just on the dashboard. It still won't reconcile with the dashboard's GANANCIA OPERATIVA: that one nets shipping against `order.total` (which *does* carry the shipping charged to the customer), a structurally different base. Both facts are stated on screen and on the printed sheet.

**The current month mixes two windows.** `report.totalRevenue` stops at today, but `ExpenseMonth.total` covers the *whole* month (the backend generates the month's future charges too), so net profit reads as a collapse that never happened — often negative on the 2nd. The figure is still shown, but labelled, on screen and in print — same rule as `shippingWindowLabel` in Gastos: label the partial window, never hide it.
