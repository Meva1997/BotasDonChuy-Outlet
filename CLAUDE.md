...
- **Testing** — Jest + React Testing Library + user-event (`next/jest`). See "Testing" below for setup, gotchas and coverage by area.
...
`jest.setup.ts` stubs: `window.matchMedia` (framer-motion's `useReducedMotion()`), `window.IntersectionObserver` (framer-motion's `whileInView`, e.g. `Hero.tsx`/`Footer.tsx`), `window.scrollTo` (jsdom implements it but throws "Not implemented" — framer-motion calls it while measuring `height: "auto"` for `NavHeader`'s mobile menu), and a bubble-phase `click` listener on `document` that swallows real `<a href>` navigation (jsdom can't navigate between documents) while letting hash links (`#…`) through. `testMatch` is scoped to `**/*.test.{ts,tsx}` — not "anything under `__tests__/`" — so fixtures/helpers can live alongside specs.

Two gotchas that cost real debugging time:
- **`jest.mock("@/...")` does not resolve.** Next's alias transform only rewrites real `import`/`require` specifiers, not the string literal passed to `jest.mock()`, which goes through Jest's own resolver (no `@/*` mapper configured there). Use a relative path in `jest.mock()` calls even though the rest of the file imports the same module via `@/`.
- **TanStack Query notifies subscribers on a macrotask.** Asserting *absence* right after a query state change (e.g. "a failed refetch must NOT show the error block") needs a `setTimeout(…, 0)` flush inside the same `act()` — without it the assertion runs against the pre-change DOM and passes either way (see `OutletView.test.tsx`'s refetch case).
...
**Real bugs caught while writing specs** (kept because the pattern recurs — worth recognizing on sight):
- **Stale closure inside a self-triggered event handler**, twice: `CodeInput.tsx`'s `handleFocus` read `value` from a stale render closure inside the synchronous `focus` event it triggers itself (typing a code digit-by-digit bounced back to the same box instead of advancing); `ProductForm.tsx`'s unmount cleanup for `blob:` previews closed over `newImages` from the mount render (always empty), so it never revoked anything added mid-session. Both fixed with a ref kept in sync at commit time / by a second effect.
- **Field left mounted-but-invalid across a schema branch**: `ExpenseForm`'s "Fecha de término" isn't unregistered by react-hook-form when `frequency` becomes `"once"`, so a previously-typed `endsAt` silently kept failing `superRefine` on submit with zero visible feedback. Fixed by clearing it in a `useEffect` the moment `frequency` changes.
- **Structural sharing defeats a "did anything change" flag**: `OrdersSection`'s polling-toast suppressor (`isManualRefreshRef`) was cleared by an effect keyed on `[data, …]`, but TanStack Query keeps the same `data` reference when a refetch returns identical content — so a no-op manual refresh left the flag armed and swallowed the *next* real toast. Fixed by adding `dataUpdatedAt` to the deps.
- **Unlabeled form fields**: both `ConfigSection` cards had `<label>`s that were mere siblings of their inputs (no `htmlFor`/nesting) — no accessible name, clicking didn't focus. Surfaced by `getByLabelText` failing; fixed with `htmlFor`/`id`.

**Known but deliberately not fixed**: `Cart.tsx`'s "Cargando..." checkout state can never render — `setNavigating(true)` and `closeCart()` fire in the same synchronous handler, so `AnimatePresence` animates the panel's exit using the tree from *before* the click. Leaving the cart open during navigation is a UX call outside a test pass's scope — see `components/ui/__tests__/README.md`.
...
components/
  home/           # NavHeader, Hero, Footer, CategoryCard, NewProducts. Hero fetches real per-category
  ...
  outlet/         # OutletView (catalog listing) + OutletCard + EmptyState + OutletSkeleton (Suspense
                  ...
                  #   filters/sorts/trims client-side (would break pagination). The categoría select
                  #   always renders, even on /botas /sombreros /ropa (where it'd otherwise be
                  #   redundant with the route) — switching it there navigates to the target category's
                  #   own route (or /outlet for "Todas"), carrying over the rest of the query params,
                  #   instead of writing a categoria param the route already fixes. URL is the source of
  ...
  legal/          # TermsConditions, PrivacyPolicy, ShippingInfo — static, sobre LegalLayout.
                  ...
                  #   Términos §5 declara el uso de IA en las fotos: la foto es SIEMPRE de la pieza
                  #   real y su estado actual, y la edición (incluida la asistida por IA) se limita al
                  #   entorno de la toma (fondo, luz, sombras, encuadre, color) sin tocar el producto
                  #   — jamás disimula desgastes, marcas ni detalles de outlet. Es una declaración
                  #   sobre cómo se producen las fotos, no sobre el código: si ese flujo cambia
                  #   (fotos de catálogo del fabricante, retoque sobre la pieza), §5 cambia con él o
                  #   pasa a ser publicidad engañosa (LFPC art. 32)
  about/          # AboutUs — static "Sobre nosotros", linked from footer. `#ubicacion` on its
                  #   location section is NewProducts.tsx's "Cómo llegar" scroll target (globals.css
                  #   sets `scroll-behavior: smooth`, neutralized under reduced-motion)
